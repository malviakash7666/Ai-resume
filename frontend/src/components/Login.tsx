...
export const Login = () => {
  ...
  const { login, user } = useAuth(); // 👈 user state nikaali check karne ke liye
  const navigate = useNavigate();

  // 🛡️ Pro-Check: Agar user logged-in hai toh login page bypass karo
  useEffect(() => {
    if (user) {
      navigate("/dashboard");
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4 font-sans">
      ...
      >
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-400">Manage your AI resume analysis</p>
        </div>
        ...
        <button 
          ...
          className={`w-full py-3.5 rounded-xl font-black text-white text-sm transition-all shadow-lg ${
            loading ? 'bg-indigo-300' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-100 active:scale-[0.98]'
          }`}
        >
